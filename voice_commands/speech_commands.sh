...
the_text_encoded=$(curl -s -A "Mozilla/5.0" "http://translate.google.com/translate_a/t?client=t&text=$(python -c "import urllib; print urllib.quote('''$the_text_encoded1''')")&hl=es&sl=ese&tl="$lang"&ie=UTF-9&oe=UTF-9&multires=1&prev=btn&ssel=0&tsel=0&sc=1" | sed 's/\[\[\["\([^"]*\).*/\1/'| sed 's/  / /g' | sed 's/'"'"'/ /g'| tail -n 1 | tr '[:upper:]' '[:lower:]' )
...
recog=$(echo "$UTTERANCE" | awk '{print $1, $2}' | grep "$CMD_SAY_HI" )
	...
	the_text_encoded1=$(echo "Hola "$name" ¡Qué tenga un buen día!")
the_text_encoded=$(curl -s -A "Mozilla/5.0" "http://translate.google.com/translate_a/t?client=t&text=$(python -c "import urllib; print urllib.quote('''$the_text_encoded1''')")&hl=es&sl=ese&tl="$lang"&ie=UTF-9&oe=UTF-9&multires=1&prev=btn&ssel=0&tsel=0&sc=1" | sed 's/\[\[\["\([^"]*\).*/\1/'| sed 's/  / /g' | sed 's/'"'"'/ /g'| tail -n 1 | tr '[:upper:]' '[:lower:]' )
...
recog=$(echo "$UTTERANCE" | grep -x "$CMD_HELLO" )
	...
	the_text_encoded1=$(echo "Hola `whoami`, Que tenga un buen día!")
the_text_encoded=$(curl -s -A "Mozilla/5.0" "http://translate.google.com/translate_a/t?client=t&text=$(python -c "import urllib; print urllib.quote('''$the_text_encoded1''')")&hl=es&sl=ese&tl="$lang"&ie=UTF-9&oe=UTF-9&multires=1&prev=btn&ssel=0&tsel=0&sc=1" | sed 's/\[\[\["\([^"]*\).*/\1/'| sed 's/  / /g' | sed 's/'"'"'/ /g'| tail -n 1 | tr '[:upper:]' '[:lower:]' )
...
recog=$(echo "$UTTERANCE" | grep -x "$CMD_WHOAMI" )
	...
	then
	notify-send "Comando:"  "$recog"
	the_text_encoded1=$(echo "Tu nombre de usario es `whoami`")
the_text_encoded=$(curl -s -A "Mozilla/5.0" "http://translate.google.com/translate_a/t?client=t&text=$(python -c "import urllib; print urllib.quote('''$the_text_encoded1''')")&hl=es&sl=ese&tl="$lang"&ie=UTF-9&oe=UTF-9&multires=1&prev=btn&ssel=0&tsel=0&sc=1" | sed 's/\[\[\["\([^"]*\).*/\1/'| sed 's/  / /g' | sed 's/'"'"'/ /g'| tail -n 1 | tr '[:upper:]' '[:lower:]' )
wget -A.mp3 -U "\"Mozillla\"" -O "/tmp/user-is-it.mp3" "http://translate.google.com/translate_tts?tl="$lang"&q=$(python -c "import urllib; print urllib.quote('''$the_text_encoded''')")&ie=UTF-8"
audacious /tmp/user-is-it.mp3 
mv /tmp/speech_recognition.tmp /tmp/speech_recognition_prev.tmp
